...
- Реализация на stdlib; рендер — SVG (не matplotlib). Метрики калибруются на размеченных
  треках (§6 спеки). Углы лавировки/повороты надёжны; сегментация ног и оценка TWD —
  ориентировочные. %-of-polar, VMC к знакам и route-efficiency — будущее (нужны polar
  и координаты знаков).
