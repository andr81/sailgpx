...
6. **Схема дистанции.** Если пользователь дал официальную схему дистанции (порядок
   знаков, флаги) — скопируй её в `files/course.png` и вставь в `## Дистанция`:
   `![[files/course.png]]`. Отдельный скриншот акватории с треком НЕ нужен — карту
   «спутник + трек по скорости» (`files/track-map.svg`) автоматически рисует
   `/sail-analyze` (см. тот скилл), она и показывает береговую зависимость.
